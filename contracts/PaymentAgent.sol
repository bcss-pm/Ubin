...
contract PaymentAgent is Owned { // Regulator node (MAS) should be the owner
  ...
  function doHash(uint256 _a) public {
    bytes32 result = keccak256(_a); //Seb solc 0.4.24: This function only accepts a single "bytes" argument. Please use "abi.encodePacked(...)" or a similar function to encode the data.
  ...
  function doHashBytes2(bytes32 _a, bytes32 _b) public {
    bytes32 result = keccak256(_a, _b); //Seb solc 0.4.24: This function only accepts a single "bytes" argument. Please use "abi.encodePacked(...)" or a similar function to encode the data.
